...
public class IQ_10_6_SortBigFile {
	...
	int findOpenNumber(String filename) throws FileNotFoundException {
		int rangeSize = (1 << 20); // 20^20 bits (2^17 bytes)
	...
	int[] getCountPerBlock(String filenmae, int rangeSize) throws FileNotFoundException {
	...
	byte[] getBitVectorForRange(String filenanme, int blockIndex, int rangeSize) throws FileNotFoundException {
		...
		while (in.hasNextInt()) {
			int value = in.nextInt();
			/* If the number is inside the block that's missing number, we record it */
